...
4. After the last chunk, run the built-in `/security-review` on the branch and hand its findings to the verifier as input. Then spawn `verifier` with the full diff, the plan, the screens file, and the security findings. It hunts: falsy values rendered, logic in JSX, components over 400 lines, secrets in client code, hallucinated packages (check every dependency exists on npm), unused abstractions.
