...
## Steps
1. Start the app in the background as a PRODUCTION build (`vite build` then `vite preview`, or the framework's equivalent), because that is what strangers get; dev servers add StrictMode double-effects and HMR that change timing. Use the dev server only when a fix needs fast iteration, and confirm on the production build before the phase closes. For an extension, build to `dist/` and load it unpacked.
