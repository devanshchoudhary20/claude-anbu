...
4. Compound: append a dated entry to `~/developer/personal/claude-anbu/LEARNINGS.md` (what the checkpoints kept flagging, what a phase got wrong, what the stack needed that the plan missed, in 3-6 lines). THINK reads this file on every mission so the next plan starts smarter.
5. Update the vault: idea file to `status: shipped` with `shipped_url`, INDEX.md row moved to Shipped, two-line "what I learned". Commit the vault.
6. Present the kit. Stop. The user posts.
